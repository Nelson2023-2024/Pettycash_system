...
from finance.default import (
    get_default_expense_category,
    get_default_expense_submitted_event,
    get_default_pending_status,
    get_default_topup_requested_event,
)
...
class ExpenseRequest(BaseModel):
    ...
    mpesa_phone = models.CharField(
        max_length=20, blank=True, verbose_name=_("M-Pesa Phone")
    )
    ...
    )  # store approved_by, timestamps, comments, etc.
...
class TopUpRequest(BaseModel):
    ...
    request_reason = models.CharField(
        max_length=255, blank=True, null=True, verbose_name=_("Request Reason")
    )
    ...
    decision_reason = models.CharField(
        max_length=255, blank=True, verbose_name=_("Decision Reason")
    )
    amount = models.DecimalField(
        max_digits=15, decimal_places=2, default=0, verbose_name=_("Amount")
    )
...
class DisbursementReconciliation(BaseModel):
    ...
    metadata = models.JSONField(
        default=dict, blank=True, null=True, verbose_name="Metadata"
    )
    ...
    def __str__(self):
        return (
            f"Reconciliation for {self.expense_request.id} | Status: {self.status.name}"
        )
...
class MpesaTransactionCost(BaseModel):
    ...
    min_amount = models.DecimalField(
        max_digits=10, decimal_places=2, verbose_name=_("Minimum Amount")
    )
    max_amount = models.DecimalField(
        max_digits=10, decimal_places=2, verbose_name=_("Maximum Amount")
    )
    cost = models.DecimalField(
        max_digits=8, decimal_places=2, verbose_name=_("Transaction Cost")
    )
    is_active = models.BooleanField(default=True, verbose_name=_("Is Active"))

    class Meta:
        db_table = "mpesa_transaction_costs"
        verbose_name = _("M-Pesa Transaction Cost")
        verbose_name_plural = _("M-Pesa Transaction Costs")
        ordering = ["min_amount"]
